...
- 确定何时回收内存
    - 程序调用System.gc时可以触发Minor GC，但在大多数情况下是系统自动决定，决定的依据是对象在Eden区中分配且没有足够空间时，会触发一次Minor GC；在Minor GC时，如果发现Survivor区的“From”区中存在达到晋升老年代（Tenured区）的年龄的对象时，将会触发一次Full GC/Major GC。
    （其中Minor GC采用的是“复制-清理”算法，而Full GC/Major GC采用的则是“标记-整理”或“标记-清理”算法。）
- 执行回收内存
    - 回收时采用的是分代回收机制，对于新生代采用“复制-清理”算法，需要分配担保，对于老年代采用“标记-整理”或“标记-清理”算法，无需分配担保。

详细的GC工作流程如下：  
- 程序调用System.gc时可以触发Minor GC，但在大多数情况下是系统自动决定，决定的依据是对象在Eden区中分配且没有足够空间时，会触发一次Minor GC，标记Eden、Survivor区中对象是否可用，然后将Eden区的可用（存活）对象复制到Survivor区的“To”区。
- 接下来会处理Survivor区的“From”区，将其中达到晋升老年代（Tenured区）的年龄的可用（存活）对象，通过触发一次Full GC/Major GC转移到Tenured区。
- Full GC/Major GC中还会对Tenured、Permanent区中的对象是否可用进行标记，然后清理不可用对象，整理可用对象。
- 在该次Minor GC中接下来将Survivor区的“From”区中未达到晋升老年代（Tenured区）的年龄的可用（存活）对象复制到Survivor区的“To”区。
- 以上工作完成后，将Eden区和Survivor区的“From”区清空，最后将Survivor区的“From”区和“To”区位置置换（“To”区变“From”区、“From”区变“To”区），保证Survivor区的“To”区始终为空。
- 每次Minor GC会一直重复这样的过程，直到Survivor区的“To”区被填满，Survivor区的“To”区被填满之后，会将其中所有可用（存活）对象转移到Tenured区。
...
## JVM配置参数有哪些，各有什么作用？
-XX:SurvivorRatio：Eden Space 与 Survivor Space 的内存大小比例，默认8:1。
-XX:+PrintGCDetails：指定虚拟机在发生垃圾回收时打印内存回收日志。
-XX:MaxTenuringThreshold：Full GC/Major GC对象年龄的阈值。默认15。
