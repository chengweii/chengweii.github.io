...
通过Iterator遍历时不可通过Iterator.remove()之外的其他方法进行更新操作，否则会抛出ConcurrentModificationException，注意对集合类进行foreach操作也是通过Iterator实现的。通过索引下标的方式遍历（for(index=0;index<list.size();index++){list.get(index);list.remove(index);index--;}）时则可以，但要注意删除元素会导致集合发生变化，遍历索引下标也要随之调整，否则可能无法正确遍历完集合。
